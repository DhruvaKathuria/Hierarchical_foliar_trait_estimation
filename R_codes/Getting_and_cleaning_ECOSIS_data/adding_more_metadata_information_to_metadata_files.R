...
add_metadata_function = function(dataset_name, variable_name1, units_for_variable_name1)
{
  ...
  }else
  {
    ...
    {
      metadata_arrow_version$manufacturer = rep(instrument_manufacturer_info_global_list[[dataset_name]], 
                                                nrow(metadata_arrow_version))
      metadata_arrow_version$model = rep(instrument_model_info_global_list[[dataset_name]], 
                                         nrow(metadata_arrow_version))
    }
...
mainDir = mainDir <- "data/raw_data"
trait_name1 = "Nitrogen" # the names of the traits are given in 
                         # supporting_R_functions/"trait_and_sample_id_Database_for_ECOSIS_Data.R" 
                         # under "trait_vector_list"

# Analysis start ----------------------------------------------------------

## IMP: Check "Steps_to_Add_Metadata_for_a_Trait_in_a_dataset.R" 
## to see whether you have the names list associated with trait.
## If not, follow the steps outlined there.

source(paste("R_codes/supporting_R_functions", 
             "getting_traits_data.R", 
             sep = "/"))
...
## Datasets which have issues (look up Github issues),
## for specific issues why a dataset wasnt selected, 
## load the metadata_arrow_version.parquet using read_parquet 
## and type comment(metadta_arrow_version) 
...
if(!(dataset_name1 %in% datasets_not_to_take))
{
  ...
  ## IMP NOTE Go through the *Steps_to_Add_Metadata_for_a_Trait_in_a_dataset.R* for 
  ## this trait and dataset combination
  ...
  metadata_arrow_version =  add_metadata_function(dataset_name1, 
                                                  trait_name1, 
                                                  trait_name1_unit)
  ...
  {
    metadata_arrow_version  = data.frame()
  }

## Add a comment below 
## if you want to add a comment to the dataset; otherwise keep it to NULL
