...
get_wiki_data = function(scientific_name, wiki_section_number)
{
  wiki_url = paste0("https://en.wikipedia.org/wiki/" , 
                    sub(" ", "_", scientific_name))
  wiki_read = readLines(wiki_url, 
                        encoding = "UTF-8")
  parsed_wiki = htmlParse(wiki_read, 
                          encoding = "UTF-8")
...
get_final_classification_for_species <- function(scientific_name, classification_form)
{
  ...
  for(wiki_section_number in 1:length(wiki_text_whole))
  {
    ...
    if(!(length(out1) == 0 | 
         out1 == "Data unavailable" | 
         out1 == "More than one matching set in Wiki, check manually")) break
  }
  out1
}
...
get_extra_metadata = function(index1, classification_form)
{
  ...
  metadata_arrow_version = read_parquet(paste0(file.path(mainDir, x), 
                                               "/", 
                                               "metadata_updated.parquet"))
  
  
  out = sapply(names(table(metadata_arrow_version$genus_species1)), 
               function(scientific_name, classification_form1 = classification_form)
               {return(tryCatch(get_final_classification_for_species(scientific_name, classification_form1), 
                   error=function(e) "Error in extraction, check"))}
  )
  closeAllConnections()
  out
}
...
{
  out1 = unlist(sapply(1:length(datasets_to_take_for_trait), 
                       function(i) get_extra_metadata(i, classification_names[ii] )))
  if (ii == 1) 
  {
...
out_mat2 = readr :: read_csv(paste0("data/Species_data/", "Species_attribute_data_Dhruva.csv"))
out_mat3 = rbind(data.frame(out_mat1), out_mat2)
out_mat3 = unique(out_mat3)
readr:: write_csv(data.frame(out_mat3), paste0("data/Species_data/", "Species_attribute_data_Dhruva.csv"))
