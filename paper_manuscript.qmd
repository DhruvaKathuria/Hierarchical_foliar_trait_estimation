...
Here the dimension of $x_i$ is $p = 2002$. $\boldsymbol{\beta}$ denotes the corresponding regression coefficients for $\boldsymbol{x_i}$, and $\sigma^2$ is the noise variance. Let the training data for the regression model i.e. the $n$ observations of both the trait and spectra be denoted by $\mathcal{D}$.
...
It is an extension of the horseshoe prior [@carvalho2010] which has been widely used in high-dimensional regression.\
...
The thick Cauchy-tails for $\lambda_j$ allow some of the regression coefficients to escape this shrinkage towards zero.\
...
This helps if the regression coefficients are weakly identified and also speeds up the sampling during posterior parameter inference using Markov Chain Monte Carlo (MCMC) methods.\
