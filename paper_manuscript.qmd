...
Think about what figures need to be included when discussin the results. A good idea will be to look at the proj pred paper bayesplot paper bayesian analysis paper by Gelman

#### Posterior predictive checks

Bayesian analysis provides us with a nice way to assess how the model performs via posterior predictive checks. What are posterior predictive checks?

### Parameter inference; show parameter correlation etc.

The $\hat{r}$ values determine whether the parameters of a model have converged or not.

A value of $\hat{r}$ equal to 1 implies that the parameter has converged.

In our analysis, we find that for many variables, the $\hat{r}$ values vary from <!--#  --> to <!--#  -->.

This is a well known issue with the HMC sampler when dealing with highly correlated inputs.

This happens for spectral bands that are highly correlated for each other.

Consider for instance, the reflectance values for wavelengths $400$ nm and $401$ nm.

For, the Nitrogen data, the correlation between the reflectance values in the training data is <!--#  -->.

Therefore, during MCMC samples, there will be some samples for which one of these wavelengths will have a non-zero regression coefficient while the others will be zero leading to a big mass at zero.

This does not however lead to any loss in predictive perfomance.

For highly correlated input variables such as the hyperspectral wavelengths, the $\hat{r}$ value.

Figure <!--#  --> shows the posterior parameter distribution histograms of the five wavelengths with largest absolute mean value for the regression coefficients.
...
The comparison of the mean posterior prediction plots with the
