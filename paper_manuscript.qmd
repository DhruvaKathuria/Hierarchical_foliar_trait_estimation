...
As mentioned in @sec-methods_posterior_projection, the reduced model finds a minimal subset of wavelengths (which are not necessarily unique) such that adding more wavelengths to the model will not lead to a significant increase in the predictive accuracy when compared with the full model. This subset of wavelengths is hence not unique especially for highly correlated wavelengths. We can, however, still use @fig-spectra_importance, to identify the VSWIR regions that are important for predicting a particular trait. For carotenoid, the red-edge (add citation) region comprises the most important wavelength, along with moderately important wavelengths in the visible spectrum which is in line with previous research. Additionally, we find a cluster of sensitive wavelength in the NIR from 1150 nm - 1300 nm around the known water-absorption band at 1240 nm. Carotenoid also has some important wavelengths in the SWIR region (1300 nm - 2500 nm). In this paper, we have not distinguished between the various form of leaves, for example healthy leaves vs non healthy leaves, top of canopy vs bottom of canopy etc, so these factors might influence these results. For example, carotenoid has been shown to be affected by drought stress, so this might be one of the reasons that we are seeing an affect of spectra beyond the visible spectrum on carotenoid as they are indirectly affecting the carotenoid content of the leaves.
