...
Though the full Bayesian model in @sec-full_bayesian_model is formulated to have good predictive accuracy, it makes use of all the input wavelengths to predict new data and as a result and has a high computational cost. <!-- I dropped the interpretability; I'm not convinced more vs. fewer bands is any more/less interpretable. --> We remedy this by defining a model which takes a relevant subset of the input hyperspectral wavelengths (of length $l_s$) as input while still mimicking the predictive capability of the full model. Therefore, our aim is to find a sub-model:
...
We also compared our new algorithm results against established best practices (e.g., 10.1093/jxb/err294) for estimating traits using PLSR.
For our PLSR trait estimates, we standardized the input wavelengths <!-- What does "standardized the input wavelengths" mean? Standardized the reflectance values to have mean 0 and stdev 1? If so, say that explicitly! --> and used minimization of the Predicted Residual Sum of Squares (PRESS; @allen1971) in cross validation to determine the number of orthogonal PLSR components.
