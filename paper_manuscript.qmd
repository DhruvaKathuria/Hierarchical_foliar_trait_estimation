...
Before interpreting the results, an often overlooked implicit assumption is that the important wavelengths selected by any model are dependent on the assumptions of the model. A Bayesian approach is helpful in this regard as we make explicit statements about our prior beliefs, the underlying trait distribution, and the measurement errors. This helps us to make wavelength selection claims only under the fixed assumptions. For example, in this paper, we did not account for important effect of climate, different land surface conditions, leaf health, across and within trait variation within plant functional types, site-specific measurement errors etc. on traits and assumed only the marginal effects of spectra on traits and a fixed Gaussian error term. This can cause latent effects of these factors in certain wavelengths shown to be important. A Bayesian model incorporating some or all of these covariates in addition to spectra, might give different results for important spectra. It is useful to keep this distinction in mind.
