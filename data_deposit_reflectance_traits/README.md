...
| `R_​codes/input_​parameter_​file.R` | Global parameter file sourced by the regression scripts below. Sets the trait-independent analysis options: the held-out test site (`site_​name1`, the CABO dataset), whether to also fit PLSR for comparison (`PLSR_​implementation`), whether inputs/​outputs are standardized (`scale_​x`, `scale_​y`). |
